...
def scalar_func(x,y,int_cen_x, int_cen_y, int_vel):
    scalar_field = 0
...
for i in range(0,len(x_p)):
    calculate  = scalar_func(x_p[i], intensity_centerums_x,intensity_centerums_y, intensity_values)
    scalar_field.append(calculate)

fig, ax = plt.subplots()

sc_plot = ax.scatter(x_p,y_p,c = scalar_field)   
